!function () {
	...
	var pluginManager = tinymce.util.Tools.resolve("tinymce.PluginManager"),
		...
		GetBodycolourMap = function (editor) {
			return editor.getParam("bodycolor_map", GetTextColourMap(editor))
		},
		...
		GetBodycolourRows = function (editor) {
			return editor.getParam("bodycolour_rows", GetTextColourRows(editor))
		},

		GetEmotionRows = function (editor) {
			return editor.getParam("emotion_rows", 5)
		},
		...
		GetBodycolourColumns = function (editor) {
			return editor.getParam("bodycolour_cols", GetTextColourColumns(editor))
		},

		GetEmotionColumns = function (editor) {
			return editor.getParam("emotion_cols", 1)
		},
		...
		CreateColourMapTable = function (colourColumns, colourRows, colourMap, colourPickerCallbackExistsBool, isEmotion = false) {
			...
			var colourMapWithTransparency,
				...
				CreateColourTableCell = function (colourCode, colourName) {
					...
					if (isEmotion) {
						...
						return '<td class="mce-grid-cell' + (isColourCodeTransparentBool ? " mce-colorbtn-trans" : "") + '"><div id="' + elementId + "-" + elementIdNumber++ + '" data-mce-color="' + (colourCode || "") + '" role="option" tabIndex="-1" style="' + (colourCode ? "background-color: " + colourCode + ";" : "") + (isColourCodeTransparentBool ? " line-height: 50%;" : "") + '" title="' + tinymceI18n.translate(colourName) + '">' + (isColourCodeTransparentBool ? "&#215;" : "") + "</div></td>"
					}
				};
			...
			return function () {
				...
				switch (switchVar) {
					...
					case 2:
						...
						break;
					case 3:
						colourColumns = GetBodycolourColumns(editor);
						colourRows = GetBodycolourRows(editor);
						colourMap = GetBodycolourMap(editor);
						break;
					case 4:
						colourColumns = GetEmotionColumns(editor);
						colourRows = GetEmotionRows(editor);
						colourMap = GetEmotionMap(editor);
						return CreateColourMapTable(colourColumns, colourRows, colourMap, colourPickerCallbackExistsBool, true);
				}
		...
		AddButtonsToEditor = function (editor) {
			...
			editor.addButton("bodycolour", { type: "colorbutton", tooltip: "Body Background Colour", image:"./assets/Bucket_Icon.png", panel: { role: "application", ariaRemember: !0, html: GetColourTableInfo(editor, 3), onclick: ColourCellClicked(editor, GetBackcolourColumns(editor), !1) }, onclick: ButtonClicked(editor, !1) }),
			editor.addButton("emotion", { type: "colorbutton", tooltip: "Emotion", text: "Emotion", style: "padding-left: 23px; background-image: url(\"./assets/Emotion_Small_Icon.png\"); background-repeat: no-repeat; background-position: 6% 50%; background-size: 18px 18px", format: "hilitecolor", panel: { role: "application", ariaRemember: !0, html: GetColourTableInfo(editor, 4), onclick: ColourCellClicked(editor, GetEmotionColumns(editor), !0) }, onclick: ButtonClicked(editor, !0) })
		};
