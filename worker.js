...
const getToken = async function(installationID, JWT){
    await fetch(`https://api.github.com/app/installations/${installationID}/access_tokens`, {
        method: "post",
        headers: {
...
const getOutputURL = async function(token, userID, resource) {
    ...
    for (var i = 0; i < runsResponse.workflow_runs.length; i++) { 
        ...
        if (runResource == resource && runUserID == userID){
            ...
            await fetch('https://api.github.com/repos/{owner}/{repo}/actions/runs/{run_id}/jobs', {
                method: "get",
                owner: "story-narrator",
                repo: "story-narrator-helper",
                ...
                return data.jobs[0].steps[2].name;
            });
        }
        else {
            return null;
